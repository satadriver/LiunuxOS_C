...
//wcl -bcl=com test.cpp
//wcl -bcl=DOS test.cpp

int copyfile16(char* src, char* dst) {
	FILE* fin = fopen(src, "rb");
	FILE* fout = fopen(dst, "wb+");

	char* buf = (char*)malloc(0x1000);
	if(buf == 0 || fin == 0 || fout == 0) {
		return -1;
	}
	int total = 0;
	int wlen = 0;
	int rlen = 0;
	do {
		rlen = fread(buf, 1, 0x1000, fin);
		if(rlen <= 0) {
			break;
		}
		total += rlen;
		wlen = fwrite(buf, 1, rlen, fout);

	}while (rlen > 0 && wlen > 0 && rlen == wlen);
...
int sectorWriter(unsigned long sector,char * data,int size) {
	PATDATASTRUCT pat = { 0 };
	unsigned short seg = FP_SEG(data);  /* 获取段地址 */
	unsigned short off = FP_OFF(data);  /* 获取偏移地址 */
	int result = 0;
	__asm {
		push ds
		push esi

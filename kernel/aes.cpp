...
int keyExpansion(const uint8_t* key, uint32_t keyLen, AesKey* aesKey) {
    ...
    if (keyLen != 16) {
        __printf(szout,"keyExpansion keyLen = %d, Not support.\n", keyLen);
        return -1;
    }
...
int aesEncrypt(const uint8_t* key, uint32_t keyLen, const uint8_t* pt, uint8_t* ct, uint32_t len) {
    ...
    if (NULL == key || NULL == pt || NULL == ct) {
        __printf(szout,"param err.\n");
        return -1;
    }

    if (keyLen > 16) {
        __printf(szout,"keyLen must be 16.\n");
        return -1;
    }

    if (len % BLOCKSIZE) {
        __printf(szout,"inLen is invalid.\n");
        return -1;
    }

    __memcpy((char*)actualKey, (char*)key, keyLen);

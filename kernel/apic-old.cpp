...
extern "C" void __declspec(dllexport) __kApInitProc() {
	...
	*(int*)(CPU_TOTAL_ADDRESS) = seq + 1;
	...
	__printf(szout, "ap id:%d tid:%d process:%x tss:%i64x idt:%x size:%x gdt:%x size:%d ltr:%x\r\n",
		cpuid,tid,process, tssdesc, idtbase_new.addr, idtbase_new.size,gdtbase_new.addr, gdtbase_new.size, tr_new);
